...
public class DirectionDistance {
     ...
     */
    public static boolean isPawnTwoStep(int sourceFile, int sourceRank, int targetFile, int targetRank) {
        int dFile = targetFile - sourceFile;
        int dRank = targetRank - sourceRank;
        //
        return (dFile == 0) && Math.abs(dRank) == 2;
	}
    ...
    public boolean isPawnTwoStep(boolean isWhite) {
		if (isWhite) {
			return direction == Direction.NORTH && distance == 2;
		}
		return direction == Direction.SOUTH && distance == 2;
	}
